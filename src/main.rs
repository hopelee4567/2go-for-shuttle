...
// hello_world, read_sub, create_config_files, download_files 函数保持不变
// The hello_world, read_sub, create_config_files, download_files functions remain unchanged
...
async fn run_services() {
    ...
    // 启动 Cloudflared 服务 (已修正)
    if Path::new(&format!("{}/bot", file_path)).exists() {
        // 从密钥中读取 Token 和 Tunnel ID
        let argo_auth = env::var("ARGO_AUTH").unwrap_or_default();
        let tunnel_id = env::var("TUNNEL_ID").unwrap_or_default();
